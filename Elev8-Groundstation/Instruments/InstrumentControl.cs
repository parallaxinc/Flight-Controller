...
namespace Elev8
{
    class InstrumentControl : System.Windows.Forms.Control
    {
		...
		/// <param name="ptImg">The location of the left upper corner of the image to display in the paint area in nominal situation</param>
		/// <param name="scaleFactor">Multiplication factor on the display image</param>
        protected void ScrollCounter(PaintEventArgs pe, Image imgBand, int nbOfDigits, float counterValue, Point ptImg, float scaleFactor)
        {
            int indexDigit =0;
            int digitBoxHeight = (int)(imgBand.Height/12);
            ...
            for(indexDigit = 0; indexDigit<nbOfDigits; indexDigit++)
            {
				...
				fader = 0.0;
				if(indexDigit == 0) {
					fader = counterValue - Math.Floor(counterValue);
				}
                ...
                if(prevDigit == 9)
                {
					float temp = (float)(counterValue / Math.Pow( 10, (indexDigit-1) ));
					fader = temp - Math.Floor( temp );
		...
		public Stream GetEmbeddedResourceStream( string name )
		{
